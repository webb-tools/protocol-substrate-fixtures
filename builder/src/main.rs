...
fn generate_mixer_keys<E: PairingEngine, const HEIGHT: usize>(curve: Curve, relative_path: &str) {
	...
	let (c, ..) = MixerR1CSProver::<E, HEIGHT>::setup_random_circuit(curve, [0u8; 32], rng).unwrap();
	// Generate the keys
...
fn generate_anchor_keys<E: PairingEngine, const HEIGHT: usize, const ANCHOR_CT: usize>(curve: Curve, relative_path: &str) {
	let rng = &mut test_rng();

	// Setup random circuit
	let (c, ..) = AnchorR1CSProver::<E, HEIGHT, ANCHOR_CT>::setup_random_circuit(curve, [0u8; 32], rng).unwrap();
	// Generate the keys
	let (proving_key, verifying_key) = Groth16::<E>::circuit_specific_setup(c, rng).unwrap();
...
fn generate_vanchor_keys<
	...
	const INS: usize,
	const OUTS: usize,
>(curve: Curve, relative_path: &str) where E::Fr: From<i128> {
	let rng = &mut test_rng();

	// Setup random circuit
	let c = VAnchorR1CSProver::<E, HEIGHT, ANCHOR_CT, INS, OUTS>::setup_random_circuit(curve, [0u8; 32], rng).unwrap();
	// Generate the keys
	let (proving_key, verifying_key) = Groth16::<E>::circuit_specific_setup(c, rng).unwrap();
...
fn main() {
	...
	generate_anchor_keys::<Bn254, 30, 2>(Curve::Bn254, "../fixed-anchor/bn254/x5");

	// Generate anchor keys with tree of heigth 30
	// and anchor count of 2
	// and number of inputs of 2
	// and number of outputs of 2
	generate_vanchor_keys::<Bn254, 30, 2, 2, 2>(Curve::Bn254, "../vanchor/bn254/x5");
}
